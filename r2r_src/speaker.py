...
class Speaker():
    ...
    def make_equiv_action(self, a_t, perm_obs, perm_idx=None, traj=None):
        def take_action(i, idx, name):
            if type(name) is int:       # Go to the next view
                ...
                self.env.env.sims[idx].makeAction(*tuple([[i] for i in self.env_actions[name]]))
    ...
    def from_shortest_path(self, viewpoints=None, get_first_feat=False):
        ...
        '''
        for i in range(0,img_feats.shape[1]):
            img_feats[:,i,:,:]=img_feats[:,-1,:,:]
        can_feats[:,0,:]=can_feats[:,1,:]
        for i in range(1,img_feats.shape[1]):
            can_feats[:,i,:]=0
        '''
        if get_first_feat:
